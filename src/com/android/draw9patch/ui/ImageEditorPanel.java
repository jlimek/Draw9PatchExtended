...
class ImageEditorPanel extends JPanel
{
	...
	private static CopiedPatches copiedPatches;
	...
	private void copyPatches()
	{
		...
		int[] top = GraphicsUtilities.getPixels(image, 0, 0, width, 1, null);
		int[] left = GraphicsUtilities.getPixels(image, 0, 0, 1, height, null);

		int[] bottom = GraphicsUtilities.getPixels(image, 0, height - 1, width, 1, null);
		int[] right = GraphicsUtilities.getPixels(image, width - 1, 0, 1, height, null);

		copiedPatches = new CopiedPatches(left, top, right, bottom);
		System.out.println();
	}

	private void pastePatches()
	{
		if (copiedPatches != null)
		{
			int imageWidth = viewer.getImageWidth();
			int imageHeight = viewer.getImageHeight();

			int[] top = copiedPatches.top;
			if (top.length != imageWidth)
			{
				top = resizePixels(top, top.length, 1, imageWidth - 1, 1);
			}
			for (int x = 0; x < top.length; x++)
			{
				viewer.drawPoint(x, 0, top[x]);
			}

			int[] bottom = copiedPatches.bottom;
			if (bottom.length != imageWidth)
			{
				bottom = resizePixels(bottom, bottom.length, 1, imageWidth - 1, 1);
			}
			for (int x = 0; x < bottom.length; x++)
			{
				viewer.drawPoint(x, imageHeight - 1, bottom[x]);
			}

			int[] left = copiedPatches.left;
			if (left.length != imageHeight)
			{
				left = resizePixels(left, left.length, 1, imageHeight - 1, 1);
			}
			for (int y = 0; y < left.length; y++)
			{
				viewer.drawPoint(0, y, left[y]);
			}

			int[] right = copiedPatches.right;
			if (right.length != imageHeight)
			{
				right = resizePixels(right, right.length, 1, imageHeight - 1, 1);
			}
			for (int y = 0; y < right.length; y++)
			{
				viewer.drawPoint(imageWidth - 1, y, right[y]);
			}
	...
	private void loadSupport()
	{
		try
		{
			...
			texture = new TexturePaint(checker, new Rectangle2D.Double(0, 0, checker.getWidth(), checker.getHeight()));
		} catch (IOException e)
		{
			e.printStackTrace();
		}
	...
	private void buildStatusPanel()
	{
		...
		status.add(label, new GridBagConstraints(0, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END, GridBagConstraints.NONE, new Insets(
				0, 6, 0, 0), 0, 0));
		...
		label.setText("100%");
		label.putClientProperty("JComponent.sizeVariant", "small");
		status.add(label, new GridBagConstraints(1, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END, GridBagConstraints.NONE, new Insets(
				0, 0, 0, 0), 0, 0));
		...
		status.add(zoomSlider, new GridBagConstraints(2, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
		...
		status.add(maxZoomLabel, new GridBagConstraints(3, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
				new Insets(0, 0, 0, 0), 0, 0));
		...
		label.putClientProperty("JComponent.sizeVariant", "small");
		status.add(label, new GridBagConstraints(0, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
		...
		label.setText("2x");
		label.putClientProperty("JComponent.sizeVariant", "small");
		status.add(label, new GridBagConstraints(1, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END, GridBagConstraints.NONE, new Insets(
				0, 0, 0, 0), 0, 0));
		...
		status.add(zoomSlider, new GridBagConstraints(2, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
				new Insets(0, 0, 0, 0), 0, 0));
		...
		status.add(maxZoomLabel, new GridBagConstraints(3, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
				new Insets(0, 0, 0, 0), 0, 0));
		...
		status.add(showLock, new GridBagConstraints(4, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
		...
		status.add(showPatches, new GridBagConstraints(4, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
				new Insets(0, 12, 0, 0), 0, 0));
		...
		status.add(showPadding, new GridBagConstraints(5, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_START, GridBagConstraints.NONE,
				new Insets(0, 12, 0, 0), 0, 0));

		status.add(Box.createHorizontalGlue(), new GridBagConstraints(6, 0, 1, 1, 1.0f, 1.0f, GridBagConstraints.LINE_START,
				GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));

		label = new JLabel("X: ");
		label.setForeground(Color.WHITE);
		label.putClientProperty("JComponent.sizeVariant", "small");
		status.add(label, new GridBagConstraints(7, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END, GridBagConstraints.NONE, new Insets(
				0, 0, 0, 0), 0, 0));

		xLabel = new JLabel("0px");
		xLabel.setForeground(Color.WHITE);
		xLabel.putClientProperty("JComponent.sizeVariant", "small");
		status.add(xLabel, new GridBagConstraints(8, 0, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END, GridBagConstraints.NONE, new Insets(
				0, 0, 0, 6), 0, 0));

		label = new JLabel("Y: ");
		label.setForeground(Color.WHITE);
		label.putClientProperty("JComponent.sizeVariant", "small");
		status.add(label, new GridBagConstraints(7, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END, GridBagConstraints.NONE, new Insets(
				0, 0, 0, 0), 0, 0));

		yLabel = new JLabel("0px");
		yLabel.setForeground(Color.WHITE);
		yLabel.putClientProperty("JComponent.sizeVariant", "small");
		status.add(yLabel, new GridBagConstraints(8, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END, GridBagConstraints.NONE, new Insets(
				0, 0, 0, 6), 0, 0));
	...
	// public static File mLastSaveDir;

	File chooseLastSaveFile()
	{
		if (is9Patch)
		{
		...
		} else
		{
			...
			// if (MainFrame.mLastOpenFile != null)
			// {
			// chooser = new
			// JFileChooser(MainFrame.mLastOpenFile.getParentFile());
			// } else
			// {
			...
			// {
			File file = MainFrame.lastOpenFile;
			if (file.exists())
			{
				file.delete();
			}
	...
	File chooseSaveFile()
	{
		...
		{
			JFileChooser chooser = new JFileChooser();
			if (MainFrame.lastOpenFile != null)
			{
				chooser = new JFileChooser(MainFrame.lastOpenFile.getParentFile());
			} else
			{
				chooser = new JFileChooser();
			}
	...
	private class StretchesViewer extends JPanel
	{
		...
		StretchesViewer()
		{
			...
			add(vertical, new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0,
					0, 0), 0, 0));
			add(horizontal, new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0,
					0, 0, 0), 0, 0));
			add(both, new GridBagConstraints(0, 2, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 0,
					0), 0, 0));
		}
		...
		private class StretchView extends JComponent
		{
			...
			protected void paintComponent(Graphics g)
			{
				...
				if (showPadding)
				{
					...
					g.fillRect(horizontalPadding.first, verticalPadding.first, scaledWidth - horizontalPadding.first
							- horizontalPadding.second, scaledHeight - verticalPadding.first - verticalPadding.second);
				}
	...
	private class ImageViewer extends JComponent
	{
		...
		private JButton copyPatchesButton;
		...
		ImageViewer()
		{
			...
			helpPanel.add(checkButton, new GridBagConstraints(9, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END,
			...
			copyPatchesButton = new JButton("Copy patches");
			copyPatchesButton.putClientProperty("JComponent.sizeVariant", "small");
			copyPatchesButton.putClientProperty("JButton.buttonType", "roundRect");
			copyPatchesButton.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent event)
				{
					copyPatches();
			...
			helpPanel.add(copyPatchesButton, new GridBagConstraints(7, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END,
					GridBagConstraints.NONE, new Insets(0, 0, 0, 6), 0, 0));

			JButton pastePatchesButton = new JButton("Paste patches");
			pastePatchesButton.putClientProperty("JComponent.sizeVariant", "small");
			pastePatchesButton.putClientProperty("JButton.buttonType", "roundRect");
			pastePatchesButton.addActionListener(new ActionListener()
			{
				public void actionPerformed(ActionEvent event)
				{
					pastePatches();
			...
			helpPanel.add(pastePatchesButton, new GridBagConstraints(8, 1, 1, 1, 0.0f, 0.0f, GridBagConstraints.LINE_END,
					GridBagConstraints.NONE, new Insets(0, 0, 0, 6), 0, 0));

			add(helpPanel, new GridBagConstraints(0, 0, 1, 1, 1.0f, 1.0f, GridBagConstraints.FIRST_LINE_START,
					GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));

			setOpaque(true);

			// Exact size will be set by setZoom() in
			...
			addMouseListener(new MouseAdapter()
			{
				...
				{
					// Store the button here instead of retrieving it again in
					// MouseDragged
					// below, because on linux, calling MouseEvent.getButton()
					// for the drag
					// event returns 0, which appears to be technically correct
					// (no button
					// changed state).
					...
					paint(event.getX(), event.getY(), currentButton, event);
				}
			});
		...
		private boolean corruptHorizontalPatch(Rectangle patch)
		{
			...
			{
				column = GraphicsUtilities.getPixels(image, patch.x + i, patch.y, 1, patch.height, column);
		...
		private void paint(int x, int y, int button, MouseEvent event)
		{
			int color;
			switch (button)
			{
			...
			if (((x == 0 || x == width - 1) && (y > 0 && y < height - 1)) || ((x > 0 && x < width - 1) && (y == 0 || y == height - 1)))
			{
				...
				} else if (event.isAltDown())
				{
					...
					{
						drawPoint(x, y, color);
						drawPoint(x, (height - 1) - y, color);
					} else if (y == 0 || y == (height - 1))
					{
						drawPoint(x, y, color);
						drawPoint((width - 1) - x, y, color);
					}
				} else
				{
					drawPoint(x, y, color);
		...
		private boolean checkLockedRegion(int x, int y)
		{
			...
			{
				Rectangle clip = new Rectangle(lastPositionX - 1 - zoom / 2, lastPositionY - 1 - zoom / 2, zoom + 2, zoom + 2);
				clip = clip.union(new Rectangle(oldX - 1 - zoom / 2, oldY - 1 - zoom / 2, zoom + 2, zoom + 2));
				repaint(clip);
			}
		...
		protected void paintComponent(Graphics g)
		{
			...
			if (corruptedPatches != null)
			{
				...
				{
					g2.draw(new RoundRectangle2D.Float(patch.x - 2.0f / zoom, patch.y - 2.0f / zoom, patch.width + 2.0f / zoom,
							patch.height + 2.0f / zoom, 6.0f / zoom, 6.0f / zoom));
				}
			}
		...
		private Pair<Integer> getPadding(List<Pair<Integer>> pairs)
		{
			...
			} else
			{
				...
				return new Pair<Integer>(pairs.get(0).second - pairs.get(0).first, pairs.get(index).second - pairs.get(index).first);
			}
		}
